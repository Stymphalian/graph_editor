...
export type D3NodeSelection = d3.Selection<SVGGElement, D3Node, null, undefined>;
export type D3EdgeSelection = d3.Selection<SVGLineElement, D3Edge, null, undefined>;
...
export const d3Utils = {
  // Color scales
  getColorScale: (domain: string[]) => d3.scaleOrdinal(d3.schemeCategory10).domain(domain),
  
  // Zoom behavior
  createZoom: () => d3.zoom<SVGSVGElement, unknown>()
    .scaleExtent([0.1, 10])
    .on('zoom', (event) => {
  ...
  createDrag: (simulation?: ForceSimulation) => d3.drag<SVGGElement, D3Node, unknown>()
    .on('start', (event, d) => {
  ...
  createForceSimulation: (width: number, height: number) => {
    return forceSimulation<D3Node, D3Edge>()
      .force('link', forceLink<D3Node, D3Edge>().id((d: D3Node) => d.id).distance(100))
