...
const TextPanel: React.FC<TextPanelProps> = ({
  ...
  const updateTextForNodeLabelChange = (
    _nodeLabel: string,
    oldLabel: string,
    newLabel: string
  ): void => {
  ...
  const updateTextForNodeRemove = (
    _nodeLabel: string,
    nodeLabelToRemove: string
  ): void => {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 2) {
        ...
        if (
          sourceLabel === nodeLabelToRemove ||
          targetLabel === nodeLabelToRemove
        ) {
          continue;
        }
      }
  ...
  const updateTextForEdgeRemove = (
    _edgeTuple: [string, string],
    sourceLabel: string,
    targetLabel: string,
    weight?: string
  ): void => {
    ...
    for (const line of lines) {
      ...
      if (parts.length >= 2) {
        ...
        const isMatch =
          (lineSourceLabel === sourceLabel &&
            lineTargetLabel === targetLabel) ||
          (data.type === 'undirected' &&
            lineSourceLabel === targetLabel &&
            lineTargetLabel === sourceLabel);
  ...
  const updateTextForEdgeWeightChange = (
    _edgeTuple: [string, string],
    sourceLabel: string,
    targetLabel: string,
    oldWeight: string | undefined,
    newWeight: string | undefined
  ): void => {
    const lines = graphTextContent.split('\n');
        ...
        switch (lastOperation.type) {
          case 'NODE_ADD':
            if (lastOperation.nodeLabel) {
              const newNode = data.nodes.find(
                node => node.label === lastOperation.nodeLabel
              );
              if (newNode) {
          ...
          case 'NODE_LABEL_CHANGE':
            if (
              lastOperation.nodeLabel &&
              lastOperation.previousValue &&
              lastOperation.newValue
            ) {
              updateTextForNodeLabelChange(
                lastOperation.nodeLabel,
                lastOperation.previousValue,
                lastOperation.newValue
              );
          ...
          case 'EDGE_REMOVE':
            if (lastOperation.edgeTuple && lastOperation.data) {
              ...
              updateTextForEdgeRemove(
                lastOperation.edgeTuple,
                sourceLabel,
                targetLabel,
                weight
              );
