...
describe('GraphUtils', () => {
  ...
  describe('compareGraphs', () => {
    it('should detect node additions and removals', () => {
      ...
      const nodeRemovals = result.changes.filter(
        c => c.type === ChangeType.NODE_REMOVE
      );
      const nodeAdditions = result.changes.filter(
        c => c.type === ChangeType.NODE_ADD
      );
    ...
    it('should detect edge additions and removals', () => {
      ...
      const edgeRemovals = result.changes.filter(
        c => c.type === ChangeType.EDGE_REMOVE
      );
      const edgeAdditions = result.changes.filter(
        c => c.type === ChangeType.EDGE_ADD
      );
    ...
    it('should detect graph property changes', () => {
      ...
      const typeChanges = result.changes.filter(
        c => c.type === ChangeType.GRAPH_TYPE_CHANGE
      );
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle indexing mode changes', () => {
      ...
      const indexingChanges = result.changes.filter(
        c => c.type === ChangeType.INDEXING_MODE_CHANGE
      );
    ...
    it('should handle max nodes changes', () => {
      ...
      const result = compareGraphs(
        graphWithMaxNodes,
        graphWithDifferentMaxNodes
      );

      const maxNodesChanges = result.changes.filter(
        c => c.type === ChangeType.MAX_NODES_CHANGE
      );
  ...
  describe('applyGraphChanges', () => {
    ...
    it('should apply edge weight changes', () => {
      ...
      const edge = graphWithWeightedEdge
        .getEdges()
  ...
  describe('extractLineOperationsFromText', () => {
    ...
    it('should handle complex mixed operations', () => {
      ...
      expect(
        addOps.length + modifyOps.length + removeOps.length
      ).toBeGreaterThan(0);
    });
  ...
  describe('extractDataChangesFromText', () => {
    ...
    beforeEach(() => {
      testGraphData = {
        nodes: [{ label: 'Alice' }, { label: 'Bob' }, { label: 'Charlie' }],
        edges: [
    ...
    it('should detect node label changes', () => {
      ...
      const nodeLabelChanges = result.changes.filter(
        c => c.type === ChangeType.NODE_LABEL_CHANGE
      );
      expect(nodeLabelChanges).toHaveLength(1);
    ...
    it('should detect edge modifications (source/target change)', () => {
      ...
      const removeOps = result.changes.filter(
        c => c.type === ChangeType.EDGE_REMOVE
      );
    ...
    it('should detect edge modifications with new node introduction', () => {
      ...
      const addNodeOps = result.changes.filter(
        c => c.type === ChangeType.NODE_ADD
      );
      const addEdgeOps = result.changes.filter(
        c => c.type === ChangeType.EDGE_ADD
      );
    ...
    it('should handle realistic graph data changes', () => {
      ...
      const edgeAddOps = result.changes.filter(
        c => c.type === ChangeType.EDGE_ADD
      );
    ...
    it('should handle complex graph restructuring', () => {
      ...
      const nodeGraphData: GraphData = {
        nodes: [{ label: 'Node1' }, { label: 'Node2' }, { label: 'Node3' }],
        edges: [
